...
namespace BrutalMemory
{
    ...
    public class Memory
    {
        ...
        public bool AobReplace(long address, byte[] bytePattern) // Updated to accept byte[]
        {
            try
            {
                return WriteProcessMemory(_processHandle, (IntPtr)address, bytePattern, (IntPtr)bytePattern.Length, IntPtr.Zero);
            }
            catch (Exception)
            {
                return false;
